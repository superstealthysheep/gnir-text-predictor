...
Save S5.00 [crossed out] S3.00 per week
...
""".toUpperCase().toCharArray();
    ...
    private static class Tensor {
        ...
        public float getValue(int[] indices) throws TensorException {
            if (indices.length != dimension) {
                throw new TensorException("Index depth " + indices.length + " does not equal tensor dimension " + dimension);
            }
    ...
    public static Tensor populateTensor(char[] input, YummyArray<Character> alphabet, int contextSize) throws Exception {
        ...
        Tensor denominators = new Tensor(contextSize-1, alphabet.size);
        ...
        for (int pos = contextSize-1; pos < input.length; pos++) {
            ...
            for (int i = 0; i < contextSize; i++) {
                ...
                try {
                    ...
                    for (int idx = pos-10; idx < pos+10; idx++) {
            ...
            result.setValue(index, 1 + result.getValue(index));    
            int[] contextIndex = Arrays.copyOfRange(index, 0, contextSize-1); // just `indices` with the last index ommitted
            denominators.setValue(contextIndex, 1 + denominators.getValue(contextIndex));
        }
        ...
        while (increment(contextIndex, alphabet.size) != null) { // loop through all the possible preceding contexts [0, ..., 0], [0, ..., 1], etc.
            float denom = denominators.getValue(contextIndex);
            Tensor column = result.getSubarray(contextIndex);
            for (int i = 0; i < alphabet.size; i++) {
                // float old_value = column.getValue()
                float new_value;
                if (denom != 0) {
                    new_value = (float) column.getValue(new int[] {i}) / denom;
                } else {
                    new_value = (float) 1 / alphabet.size;
                }
    ...
    public static char nextChar(char[] seed, Tensor weights, YummyArray<Character> alphabet) throws Exception {
        ...
        for (int idx = 0; idx < alphabet.size; idx++) {
            accum += column.getValue(new int[] {idx});
            if (accum >= rand) {
